...
/**
 * The server's new per-field/per-embed caps. `MAX_IMAGE_URL_RUNES` covers
 * only `image.url` and `thumbnail.url` — an author icon and a footer icon
 * stay uncapped, matching `requireHttps`'s existing image-url handling in
 * embeds.ts. `MAX_EMBED_TOTAL_LENGTH` is a whole-MESSAGE cap, summed across
 ...
 * byte-identical to `sdk/python/aurival/caps.py`'s mirrors.
 */
...
export const MAX_IMAGE_URL_RUNES = 2048;
export const MAX_EMBED_TOTAL_LENGTH = 6000;

export const CAP_FIELD_NAME_TOO_LONG = `an embed field name is at most ${MAX_FIELD_NAME_LENGTH} characters`;
export const CAP_FIELD_VALUE_TOO_LONG = `an embed field value is at most ${MAX_FIELD_VALUE_LENGTH} characters`;
export const CAP_AUTHOR_NAME_TOO_LONG = `an embed author name is at most ${MAX_AUTHOR_NAME_LENGTH} characters`;
export const CAP_FOOTER_TEXT_TOO_LONG = `an embed footer's text is at most ${MAX_FOOTER_TEXT_LENGTH} characters`;
export const CAP_IMAGE_URL_TOO_LONG = `an image url is at most ${MAX_IMAGE_URL_RUNES} characters`;
export const CAP_EMBEDS_TOO_LONG = `the embeds on a message carry at most ${MAX_EMBED_TOTAL_LENGTH} characters in total, across every title, description, field name and value, author name and footer text`;
